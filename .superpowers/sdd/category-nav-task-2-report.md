...
- Updated every `CategoryCompetitionCard` primary action to stay in the dedicated category workspace. The overview, registrations, groups, games, and results targets now use `/torneios/[tournamentId]/categorias/[categoryId]?tab=...`; no action returns to the removed event-level tabs.
- Added an explicit `categoryId` card prop at the category-route call site.
- Added the selected pair ranking to the compact workspace header, using the ranking relation already loaded for the category and displaying **Sem ranking** when no pair ranking is selected.
- Added source regressions for both review findings. The focused suite first failed both new assertions, then passed 20/20 after the fixes.
- Follow-up verification: `npx tsx --test tests/*.test.ts` passed 62/62, `npm run typecheck` passed, and `git diff --check` passed.

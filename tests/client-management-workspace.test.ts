...
test("client workspace exposes operational filters, balances and plan history", () => {
  ...
  const mergePicker = readFileSync(resolve(process.cwd(), "src/components/players/client-merge-picker.tsx"), "utf8");
  ...
  assert.match(page, /ClientMergePicker/);
  assert.match(mergePicker, /Pesquisar cliente por nome ou telefone/);
  assert.match(mergePicker, /client-merge-search-results/);

...
function toCurrencyInput(cents: number) { return formatMoneyInput(cents); }
...
export function AgendaSlotDialog({ slot, players, courts, teachers, bookingTypes, children }: { slot: AgendaSlot; players: Player[]; courts: Court[]; teachers: Teacher[]; bookingTypes: string[]; children: React.ReactNode }) {
  ...
  const addPlayerToReservation = (playerId: string) => { if (selectedParticipants.some((participant) => participant.playerId === playerId)) return; setParticipants((current) => { const emptyIndex = current.findIndex((participant) => !participant.playerId); return emptyIndex >= 0 ? current.map((participant, index) => index === emptyIndex ? { ...participant, playerId } : participant) : [...current, { ...emptyParticipant(), playerId }]; }); setPlayerSearch(""); };
  ...
  const openComandasForParticipants = () => {
    if (!slot.occurrenceId) return;
    setError("");
  ...
  }, [open, optionsOpen, quickCreateOpen]);
  ...
  return <>
    <div className="agenda-slot-entry"><button type="button" className="agenda-slot-trigger" onClick={() => setOpen(true)}>{children}</button>{slot.state === "OCCUPIED" && slot.occurrenceId ? <div className="agenda-slot-options"><button type="button" className="agenda-slot-options-trigger" aria-label="Opções do horário" aria-expanded={optionsOpen} onClick={() => setOptionsOpen((current) => !current)}>⋯</button>{optionsOpen ? <div className="agenda-slot-options-menu"><button type="button" className="agenda-slot-option-cancel" onClick={() => cancelBooking("CANCEL")}>Cancelar horário</button><button type="button" className="agenda-slot-option-free" onClick={() => cancelBooking("FREE")}>Liberar horário</button></div> : null}</div> : null}</div>
    {open ? <div className="agenda-slot-backdrop" role="presentation" onMouseDown={() => setOpen(false)}><section className="agenda-slot-dialog agenda-booking-dialog" role="dialog" aria-modal="true" aria-label="Agendamento de horário" onMouseDown={(event) => event.stopPropagation()}>
      <header><div><p className="eyebrow">{stateLabel}</p><h2>{slot.courtName}</h2><span>{slot.dateLabel} · {slot.startsAt} às {endTime}</span>{slot.state === "OCCUPIED" && slot.occurrenceId ? <div className="agenda-reservation-client"><strong>{primaryParticipantName}</strong><button type="button" className="agenda-open-comandas-button" onClick={openComandasForParticipants} disabled={pending}>Abrir comandas dos atletas</button>{slot.pendingConfirmation ? <OnlineBookingConfirmButton occurrenceId={slot.occurrenceId} /> : null}</div> : null}</div><button className="button" type="button" onClick={() => setOpen(false)}>Fechar</button></header>
      {slot.state === "UNAVAILABLE" ? <p className="form-note">Este horário está bloqueado pela configuração da quadra.</p> : <div className="agenda-booking-form">
        ...
        <div className="agenda-court-price-summary">{courtPriceEditing ? <MoneyInput valueCents={courtAmountCents} onValueCentsChange={(amount) => super12 ? setAmountPerAthlete(amount) : setCourtAmountCents(amount)} onBlur={() => setCourtPriceEditing(false)} autoFocus /> : <button type="button" onClick={() => setCourtPriceEditing(true)}>{super12 ? "Valor por atleta" : "Valor da quadra"}: R$ {formatMoneyInput(courtAmountCents)}</button>}{!super12 ? <button type="button" className="button button-primary agenda-split-button" onClick={splitEvenly}>Dividir igualmente</button> : null}</div>
        <section className="agenda-participants"><div className="agenda-participants-heading"><div><h3>{super12 ? `Atletas do Super 12 (${selectedParticipants.length})` : "Atletas"}</h3></div>{!super12 ? <button type="button" className="button button-primary agenda-add-athlete" onClick={addParticipant}>+ Adicionar atleta</button> : null}</div><div className="agenda-player-search"><input placeholder={super12 ? "Buscar e adicionar atleta..." : "Pesquisar atleta..."} value={playerSearch} onChange={(event) => setPlayerSearch(event.target.value)} />{playerSearch.trim() && !matchingPlayers.length ? <button type="button" className="button" onClick={() => { setQuickName(playerSearch); setQuickCreateOpen(true); }}>Cadastrar novo atleta</button> : null}</div>{!super12 && playerSearch.trim() && matchingPlayers.length ? <div className="agenda-player-suggestions">{matchingPlayers.filter((player) => !selectedParticipants.some((participant) => participant.playerId === player.id)).slice(0, 8).map((player) => <button type="button" key={player.id} onClick={() => addPlayerToReservation(player.id)}>{player.name}</button>)}</div> : null}

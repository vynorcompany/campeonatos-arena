  ...
  return <>
    ...
    {open ? <div className="agenda-slot-backdrop" role="presentation" onMouseDown={() => setOpen(false)}><section className="agenda-slot-dialog agenda-booking-dialog" role="dialog" aria-modal="true" aria-label="Agendamento de horário" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {slot.state === "UNAVAILABLE" ? <p className="form-note">Este horário está bloqueado pela configuração da quadra.</p> : <div className="agenda-booking-form">
        ...
        <div className="agenda-court-price-summary">{courtPriceEditing ? <MoneyInput valueCents={courtAmountCents} onValueCentsChange={(amount) => super12 ? setAmountPerAthlete(amount) : setCourtAmountCents(amount)} onBlur={() => setCourtPriceEditing(false)} autoFocus /> : <button type="button" onClick={() => setCourtPriceEditing(true)}>{super12 ? "Valor por atleta" : "Valor da quadra"}: R$ {formatMoneyInput(courtAmountCents)}</button>}{!super12 ? <button type="button" className="button button-primary" onClick={splitEvenly}>Dividir igualmente</button> : null}</div>

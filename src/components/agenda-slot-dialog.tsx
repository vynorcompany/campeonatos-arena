...
function durationLabel(value: number) { const hours = Math.floor(value / 60); const minutes = value % 60; return hours ? `${hours}h${minutes ? ` ${minutes}min` : ""}` : `${minutes} min`; }
...
export function AgendaSlotDialog({ slot, players, courts, teachers, bookingTypes, children }: { slot: AgendaSlot; players: Player[]; courts: Court[]; teachers: Teacher[]; bookingTypes: string[]; children: React.ReactNode }) {
  ...
  const [durationMinutes, setDurationMinutes] = useState(() => Math.max(15, startMinute(slot.endsAt) - startMinute(slot.startsAt)));
  ...
  const [responsiblePlayerId, setResponsiblePlayerId] = useState(slot.responsiblePlayerId ?? "");
  ...
  useEffect(() => {
    ...
    fetch(`/api/agenda/availability?courtId=${encodeURIComponent(slot.courtId)}&date=${encodeURIComponent(dateValue)}${slot.occurrenceId ? `&occurrenceId=${encodeURIComponent(slot.occurrenceId)}` : ""}`, { signal: controller.signal })
      ...
      .then((availability: { slotMinutes: number; availableMinutes: number[] }) => {
        ...
        setStartsAt((currentStart) => { const nextStartMinute = availability.availableMinutes.includes(startMinute(currentStart)) ? startMinute(currentStart) : availability.availableMinutes[0]; if (nextStartMinute == null) return currentStart; const nextDurationOptions = durationMinutesForBookingStart({ startMinute: nextStartMinute, slotMinutes: availability.slotMinutes, availableMinutes: availability.availableMinutes }); setDurationMinutes((currentDuration) => nextDurationOptions.includes(currentDuration) ? currentDuration : nextDurationOptions[0] ?? availability.slotMinutes); return minuteLabel(nextStartMinute); });
  ...
  const submit = () => {
    ...
    formData.set("courtId", slot.courtId); formData.set("courtIds", JSON.stringify(super12 ? courtIds : [slot.courtId])); formData.set("title", reservationName); formData.set("startsAt", `${dateValue}T${startsAt}`); formData.set("durationMinutes", String(selectedEndMinute - selectedStartMinute)); formData.set("bookingTypeName", bookingTypeName); formData.set("repeatUntil", fixedBooking ? repeatUntil : ""); formData.set("teacherId", lesson ? teacherId : ""); formData.set("responsiblePlayerId", fixedBooking ? responsiblePlayerId || selectedParticipants[0]?.playerId || "" : ""); formData.set("notes", notes); formData.set("participants", JSON.stringify(selectedParticipants));
  ...
  const cancelBooking = (mode: "CANCEL" | "FREE") => { if (!slot.occurrenceId) return; const action = mode === "FREE" ? "liberar somente este horário" : "cancelar a reserva"; if (!window.confirm(`Confirma ${action}? Esta ação não poderá ser desfeita.`)) return; setError(""); const formData = new FormData(); formData.set("occurrenceId", slot.occurrenceId); formData.set("mode", mode); startTransition(async () => { try { await cancelCourtBookingAction(formData); setOptionsOpen(false); setOpen(false); router.refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível cancelar o horário."); } }); };
  const openComandasForParticipants = () => { if (!slot.occurrenceId) return; setError(""); if (!selectedParticipants.length) { setError("Não é possível abrir comandas: esta reserva não possui cliente atribuído."); return; } const formData = new FormData(); formData.set("occurrenceId", slot.occurrenceId); startTransition(async () => { try { const result = await openBookingComandasAction(formData); if (result.error) { setError(result.error); return; } setOptionsOpen(false); router.push("/comandas"); router.refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível abrir as comandas."); } }); };

  return <>
    <div className="agenda-slot-entry"><button type="button" className="agenda-slot-trigger" onClick={() => setOpen(true)}>{children}</button>{slot.state === "OCCUPIED" && slot.occurrenceId ? <div className="agenda-slot-options"><button type="button" className="agenda-slot-options-trigger" aria-label="Ações da reserva" title="Ações da reserva" aria-expanded={optionsOpen} onClick={() => setOptionsOpen((current) => !current)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h8m4 0h4M4 17h4m4 0h8M12 4v6m0 4v6" /></svg></button>{optionsOpen ? <div className="agenda-slot-options-menu"><button type="button" className="agenda-slot-option-cancel" onClick={() => cancelBooking("CANCEL")}>Cancelar reserva</button><button type="button" className="agenda-slot-option-free" onClick={() => cancelBooking("FREE")}>Liberar este horário</button></div> : null}</div> : null}</div>
    {open ? <div className="agenda-slot-backdrop" role="presentation" onMouseDown={() => setOpen(false)}><section className="agenda-slot-dialog agenda-booking-dialog" role="dialog" aria-modal="true" aria-label="Agendamento de horário" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {slot.state === "UNAVAILABLE" ? <p className="form-note">Este horário está bloqueado pela configuração da quadra.</p> : <div className="agenda-booking-form">
        ...
        {fixedBooking ? <label className="agenda-booking-extra">Responsável pela reserva fixa<select value={responsiblePlayerId || selectedParticipants[0]?.playerId || ""} onChange={(event) => setResponsiblePlayerId(event.target.value)} required><option value="" disabled>Selecione um participante</option>{selectedParticipants.map((participant) => <option value={participant.playerId} key={participant.playerId}>{playerOptions.find((player) => player.id === participant.playerId)?.name ?? "Participante"}</option>)}</select><small>Recebe um aviso se outro participante ficar com saldo em aberto.</small></label> : null}

...
export function PublicSuper12({ arenaSlug, data }: { arenaSlug: string; data: Super12Data }) {
  ...
  return <section className="athlete-portal-content-panel super12-panel">
    ...
    {event ? <section className="super12-board">
      <header><div><span>{event.status === "FINISHED" ? "CONCLUÍDO" : event.format === "GROUPS" ? "FORMATO: GRUPOS" : "FORMATO: TODOS CONTRA TODOS"}</span><h3>{event.name}</h3><p>Organizado por {event.creator.name} · {boardGroups.reduce((total, group) => total + group.pairs.length, 0)} duplas.</p></div>{event.isCreator && event.status === "ACTIVE" ? <SafeActionForm action={finishSuper12Action} successMessage="Super 12 concluído. Ele não aparecerá na lista de rodadas em andamento." confirmKeyword="ENCERRAR" confirmPrompt="A classificação continuará disponível neste link, mas a rodada sairá da lista principal."><input type="hidden" name="arenaSlug" value={arenaSlug} /><input type="hidden" name="eventId" value={event.id} /><SubmitButton label="Concluir rodada" pendingLabel="Concluindo..." className="button button-small" /></SafeActionForm> : null}</header>
    ...
    </section> : <section className="super12-empty"><strong>Nenhum Super 12 em andamento</strong><p>Escolha de 4 a 12 atletas para começar uma rodada entre duplas.</p></section>}
  </section>;
}

...
function ClassesPanel({ portal, teacherId }: { portal: Portal; teacherId?: string }) {
  ...
  return <section className="athlete-portal-content-panel"><header><span>TURMAS</span><h2>Encontre sua turma</h2></header><div className="portal-class-group-list">{portal?.classGroups.length ? portal.classGroups.map((group) => <article key={group.id}><div><strong>{group.name}</strong><span>{group.teacherName}</span><small>{group.schedules.map((schedule) => `${weekdays[schedule.weekday]} ${schedule.startTime} · ${schedule.capacity} vagas`).join("  |  ")}</small></div>{group.enrolled ? <b>Você participa</b> : group.requestPending ? <b>Solicitação enviada</b> : group.available ? <SafeActionForm action={requestClassGroupAction} successMessage="Solicitação enviada para a arena."><input type="hidden" name="arenaSlug" value={portal.arenaSlug} /><input type="hidden" name="classGroupId" value={group.id} /><SubmitButton label="Solicitar vaga" pendingLabel="Enviando..." className="button button-primary button-small" /></SafeActionForm> : <b>Sem vagas</b>}</article>) : <p className="muted">Nenhuma turma disponível no momento.</p>}</div><div className="portal-teacher-picker"><strong>Agenda de professores</strong>{teachers.length ? <div>{teachers.map((teacher) => <Link className={selectedTeacher?.id === teacher.id ? "active" : ""} href={portalHref("classes", undefined, teacher.id)} key={teacher.id}>{teacher.name}</Link>)}</div> : <span>Nenhum professor ativo cadastrado.</span>}</div>{selectedTeacher ? <div className="portal-activity-list"><strong className="portal-selected-teacher">Agenda de {selectedTeacher.name}</strong>{classes.length ? classes.map((item) => <article key={item.id}><div><strong>{item.title}</strong><span>{item.when}</span></div><b>{item.status}</b></article>) : <p className="muted">Não há aulas futuras para este professor.</p>}</div> : null}</section>;
}

function TeacherManagementPanel({ portal }: { portal: Portal }) {
  const management = portal?.teacherManagement;
  const weekdays = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"];
  return <section className="athlete-portal-content-panel teacher-portal-management"><header><span>GESTÃO DO PROFESSOR</span><h2>Planos, alunos, turmas e agenda</h2></header>{management ? <div className="teacher-portal-management-grid"><article><h3>Planos</h3>{management.plans.length ? management.plans.map((plan) => <div key={plan.id}><strong>{plan.name}</strong><span>{plan.classesPerMonth} aulas/mês · {(plan.monthlyPriceCents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span></div>) : <p>Nenhum plano vinculado.</p>}</article><article><h3>Alunos ativos</h3>{management.students.length ? management.students.map((student) => <div key={student.id}><strong>{student.name}</strong><span>{student.planName} · saldo: {student.remainingClasses} aula(s)</span></div>) : <p>Nenhum aluno ativo.</p>}</article><article><h3>Minhas turmas</h3>{management.classGroups.length ? management.classGroups.map((group) => <div key={group.id}><strong>{group.name}</strong><span>{group.schedules.map((schedule) => `${weekdays[schedule.weekday]} ${schedule.startTime}: ${group.enrolledCount}/${schedule.capacity} vagas`).join(" · ")}</span>{group.students.map((student) => <div className="teacher-class-student" key={student.id}><b>{student.name}</b><SafeActionForm action={moveClassGroupStudentAction}><input type="hidden" name="arenaSlug" value={portal?.arenaSlug} /><input type="hidden" name="sourceClassGroupId" value={group.id} /><input type="hidden" name="studentId" value={student.id} /><select name="destinationClassGroupId" defaultValue=""><option value="" disabled>Mover aluno</option>{management.classGroups.filter((target) => target.id !== group.id).map((target) => <option value={target.id} key={target.id}>{target.name}</option>)}</select><SubmitButton label="Mover aluno" pendingLabel="Movendo..." className="button button-small" /></SafeActionForm><SafeActionForm action={registerClassGroupMakeupAction}><input type="hidden" name="arenaSlug" value={portal?.arenaSlug} /><input type="hidden" name="sourceClassGroupId" value={group.id} /><input type="hidden" name="studentId" value={student.id} /><select name="destinationClassGroupId" defaultValue=""><option value="" disabled>Turma da reposição</option>{management.classGroups.filter((target) => target.id !== group.id).map((target) => <option value={target.id} key={target.id}>{target.name}</option>)}</select><input name="scheduledFor" type="date" required /><SubmitButton label="Registrar reposição" pendingLabel="Salvando..." className="button button-small" /></SafeActionForm></div>)}</div>) : <p>Nenhuma turma vinculada.</p>}</article><article><h3>Agenda</h3>{management.agenda.length ? management.agenda.map((item) => <div key={item.id}><strong>{item.title}</strong><span>{item.when} · {item.status}</span></div>) : <p>Nenhuma atividade futura.</p>}</article></div> : <PortalEmpty title="Perfil de professor indisponível" detail="Peça à arena para ativar o seu cadastro de professor." />}</section>;
}

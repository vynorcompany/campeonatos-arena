...
function StudentCard({ arenaSlug, student, classGroups }: { arenaSlug: string; student: Student; classGroups: ClassGroup[] }) {
  ...
  return <details className="teacher-portal-student-row">
    ...
    <div className="teacher-student-workspace">
      ...
      {action === "balance" ? <section className="teacher-student-action-panel"><header><span>Saldo do ciclo atual</span><strong>{student.remainingClasses} <small>de {student.initialMonthlyClasses} aulas</small></strong></header><SafeActionForm action={adjustTeacherStudentBalanceAction} className="teacher-student-balance-form"><input type="hidden" name="arenaSlug" value={arenaSlug} /><input type="hidden" name="studentId" value={student.id} /><label><span>Ajuste</span><input name="classesDelta" type="number" min="-99" max="99" placeholder="+1 ou -1" required /></label><label><span>Motivo</span><input name="reason" placeholder="Ex.: crédito por reposição" required /></label><SubmitButton label="Salvar ajuste" pendingLabel="Salvando..." className="button button-small" /></SafeActionForm></section> : null}

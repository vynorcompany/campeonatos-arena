    ...
    <div className="portal-editor-workspace">
      <section className="section-card">
        ...
        <div className="client-portal-settings-list">
          {announcements.map((announcement) => <article key={announcement.id}><div><strong>{announcement.pinned ? "📌 " : ""}{announcement.title}</strong><span className={`portal-notice-status ${announcement.active ? "is-active" : ""}`}>{announcement.active ? "Aviso ativo" : "Aviso desativado"}{announcement.linkUrl ? " · Link ativo" : ""}</span><span className="portal-editor-text">{announcement.message}</span></div><div className="section-actions"><SafeActionForm action={togglePortalAnnouncementPinAction}><input type="hidden" name="announcementId" value={announcement.id} /><SubmitButton label={announcement.pinned ? "Desafixar" : "Fixar"} pendingLabel="Salvando..." className="button button-small" /></SafeActionForm><SafeActionForm action={togglePortalAnnouncementAction}><input type="hidden" name="announcementId" value={announcement.id} /><SubmitButton label={announcement.active ? "Desativar aviso" : "Ativar aviso"} pendingLabel="Salvando..." className="button button-small" /></SafeActionForm><button className="button button-danger button-small" type="button" onClick={() => setAnnouncementToDelete(announcement)}>Excluir</button></div></article>)}
      ...
      <section className="section-card">
        ...
        <div className="portal-event-post-list">
          {posts.map((post) => <article key={post.id}><img src={post.imageUrl} alt={`Imagem do evento ${post.title}`} /><div><strong>{post.pinned ? "📌 " : ""}{post.title}</strong><span className="portal-editor-text">{post.caption}</span>{post.linkUrl ? <small>Link ativo</small> : null}<form action={togglePortalEventPostAction}><input type="hidden" name="eventPostId" value={post.id} /><label className="control-toggle"><input type="checkbox" defaultChecked={post.active} onChange={(event) => event.currentTarget.form?.requestSubmit()} /><span aria-hidden="true" />Exibir no Portal</label></form><div className="section-actions"><SafeActionForm action={togglePortalEventPostPinAction}><input type="hidden" name="eventPostId" value={post.id} /><SubmitButton label={post.pinned ? "Desafixar" : "Fixar"} pendingLabel="Salvando..." className="button button-small" /></SafeActionForm><button className="button button-small" type="button" onClick={() => setPostToEdit(post)}>Editar evento</button><button className="button button-small" type="button" onClick={() => setPostToReplace(post)}>Trocar imagem</button><button className="button button-danger button-small" type="button" onClick={() => setPostToDelete(post)}>Excluir</button></div></div></article>)}
          {!posts.length ? <p className="muted">Nenhum evento publicado.</p> : null}
        </div>
      </section>
      {dialog === "notice" ? <Dialog title="Novo aviso" close={() => setDialog(null)}><SafeActionForm action={createPortalAnnouncementAction} className="portal-editor-form" resetOnSuccess successMessage="Aviso publicado." onSuccess={() => setDialog(null)}><div className="field form-full"><label>Título<input name="title" required /></label></div><div className="field"><label>Início da publicação<input name="startsAt" type="datetime-local" /></label></div><div className="field"><label>Fim da publicação<input name="endsAt" type="datetime-local" /></label></div><div className="field form-full"><label>Link do aviso <small>Opcional. Use um endereço iniciado por https://</small><input name="linkUrl" type="url" inputMode="url" placeholder="https://exemplo.com" /></label></div><div className="field form-full"><label className="control-toggle"><input name="pinned" type="checkbox" /><span aria-hidden="true" /><em>Fixar no topo do feed</em></label></div><div className="field form-full"><label>Aviso<textarea name="message" rows={6} required placeholder="Use **texto** para negrito." /></label></div><footer className="portal-editor-form-footer"><SubmitButton label="Publicar aviso" pendingLabel="Publicando..." className="button button-primary" /></footer></SafeActionForm></Dialog> : null}
      ...
      {announcementToDelete ? <Dialog title="Excluir aviso" close={() => setAnnouncementToDelete(null)}><SafeActionForm action={deletePortalAnnouncementAction} className="portal-editor-form" successMessage="Aviso excluído." onSuccess={() => setAnnouncementToDelete(null)}><input type="hidden" name="announcementId" value={announcementToDelete.id} /><p className="form-full muted">Excluir “{announcementToDelete.title}” do portal? Esta ação não pode ser desfeita.</p><footer className="portal-editor-form-footer"><SubmitButton label="Excluir aviso" pendingLabel="Excluindo..." className="button button-danger" /></footer></SafeActionForm></Dialog> : null}
      {postToDelete ? <Dialog title="Excluir evento" close={() => setPostToDelete(null)}><SafeActionForm action={deletePortalEventPostAction} className="portal-editor-form" successMessage="Evento excluído." onSuccess={() => setPostToDelete(null)}><input type="hidden" name="eventPostId" value={postToDelete.id} /><p className="form-full muted">Excluir “{postToDelete.title}” do portal? Esta ação não pode ser desfeita.</p><footer className="portal-editor-form-footer"><SubmitButton label="Excluir evento" pendingLabel="Excluindo..." className="button button-danger" /></footer></SafeActionForm></Dialog> : null}
    </div>
  );
}

...
import {
  ...
  type Account,
  type Option,
} from "@/components/finance/accounts-ledger-parts";
...
export function AccountsLedger({
  ...
  const selectableEntries = entries.filter((entry) => entry.status !== "VOIDED" && entry.source !== "AGENCY_SUBSCRIPTION");
  ...
  const openEntry = (entry: Account) => {
    if (entry.source === "AGENCY_SUBSCRIPTION") { setNotice("Fatura vinculada à agência. Use o link de pagamento; a baixa acontece automaticamente após a confirmação."); return; }
  ...
  return (
    <div className="accounts-ledger stack-md">
      ...
      <form method="get" className="accounts-filters">
        ...
        {type === "REVENUE" ? <><select name="productId" defaultValue={String(filters.productId ?? "")}><option value="">Produto</option>{products.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select name="planId" defaultValue={String(filters.planId ?? "")} onChange={(event) => event.currentTarget.form?.requestSubmit()} aria-label="Filtrar por plano ou por todos os planos do professor"><option value="">Plano/pacote</option><PlanSelectOptions plans={plans} includeTeacherAll /></select></> : null}
      ...
      <section className="accounts-ledger-list" aria-label={title}>
        ...
        {entries.map((entry) => {
          ...
          <article className={`accounts-ledger-row accounts-ledger-row-clickable${overdue ? " accounts-ledger-row-overdue" : ""}`} key={entry.id} role="button" tabIndex={0} onClick={() => openEntry(entry)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openEntry(entry); }}>
            <span className="accounts-ledger-selection"><input type="checkbox" aria-label={`Selecionar ${entry.description}`} checked={selectedEntryIds.has(entry.id)} disabled={entry.status === "VOIDED" || entry.source === "AGENCY_SUBSCRIPTION"} onClick={(event) => event.stopPropagation()} onChange={() => toggleEntrySelection(entry.id)} /></span><span>{date(entry.dueDate)}</span><strong>{type === "REVENUE" && entry.counterpartyName !== "Não informado" ? <Link href={`/jogadores?q=${encodeURIComponent(entry.counterpartyName)}`} onClick={(event) => event.stopPropagation()}>{entry.counterpartyName}</Link> : entry.counterpartyName}</strong><span>{entry.category}</span><span>{entry.description}{entry.comanda ? <Link className="account-comanda-link" href={`/comandas/${entry.comanda.id}`} onClick={(event) => event.stopPropagation()}>Ver comanda · {entry.comanda.code}</Link> : null}</span>
            ...
            <span className="accounts-ledger-actions">
              {type === "EXPENSE" && entry.onlinePaymentUrl && entry.status === "PENDING" ? <a className="button button-small button-primary" href={entry.onlinePaymentUrl} target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()}>Pagar online</a> : null}
              {entry.source !== "AGENCY_SUBSCRIPTION" ? <>
                {entry.status === "PENDING" ? <><button type="button" className="button button-small button-primary" onClick={(event) => { event.stopPropagation(); setPaymentEntry(entry); }}>{actionLabel}</button>{type === "REVENUE" ? <>{entry.onlinePaymentUrl ? <button type="button" className="button button-small" onClick={(event) => { event.stopPropagation(); setOnlineCharge({ method: entry.onlinePaymentQrCode ? "BOLETO" : "PIX", url: entry.onlinePaymentUrl, code: entry.onlinePaymentQrCode }); }}>Ver cobrança</button> : <><button type="button" className="button button-small" disabled={pending} onClick={(event) => { event.stopPropagation(); generateOnlineCharge(entry, "PIX"); }}>Gerar PIX</button><button type="button" className="button button-small" disabled={pending} onClick={(event) => { event.stopPropagation(); generateOnlineCharge(entry, "BOLETO"); }}>{entry.onlinePaymentMethod === "BOLETO" ? "Emitir boleto" : "Gerar boleto"}</button></>}</> : null}</> : null}
                {entry.status !== "VOIDED" ? <button type="button" className="button button-small" onClick={(event) => { event.stopPropagation(); setVoidEntry(entry); }}>Estornar</button> : null}
                {canDeleteEntries && entry.status !== "VOIDED" ? <button type="button" className="button button-small button-danger" onClick={(event) => { event.stopPropagation(); if (!window.confirm("Excluir este lançamento? Ele será removido da operação, mas permanecerá registrado para auditoria.")) return; const form = new FormData(); form.set("entryId", entry.id); run(() => deleteFinancialEntryAction(form), () => {}); }}>Excluir</button> : null}
              </> : null}
            </span>
          </article>
          );

        ...
        <div className="command-modal-backdrop" onMouseDown={() => setNewEntryOpen(false)} role="presentation">
          <section className="financial-entry-modal" role="dialog" aria-modal="true" aria-label="Novo lançamento" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <form onSubmit={(event) => {
              ...
              {type === "REVENUE" && counterpartyName.trim() ? <div className="supplier-suggestions form-full" aria-label="Selecionar cliente cadastrado">
                {matchingClients.map((client) => <button key={client.id} type="button" onClick={() => setCounterpartyName(client.name)}>{client.name}</button>)}
              </div> : null}

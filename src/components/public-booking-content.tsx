...
export async function PublicBookingContent({ arenaSlug, date, embedded = false }: PublicBookingContentProps) {
  ...
  const [occurrences, pendingReservations] = await withArenaTransaction(arena.id, (tx) => Promise.all([
    ...
    currentClient ? tx.scheduleOccurrence.findMany({ where: { arenaId: arena.id, sourceType: "ONLINE_BOOKING", status: "PENDING_PAYMENT", participants: { some: { playerId: currentClient.playerId } }, endsAt: { gte: selectedDate } }, include: { occurrenceCourts: true }, orderBy: { createdAt: "desc" }, take: 1 }) : Promise.resolve([]),
  ]));

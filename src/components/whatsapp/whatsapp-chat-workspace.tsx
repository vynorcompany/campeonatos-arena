...
export function WhatsAppChatWorkspace({ conversations, connected, clients, slaMinutes }: { conversations: Conversation[]; connected: boolean; clients: Client[]; slaMinutes: number }) {
  ...
  useEffect(() => { let version = ""; const refresh = async () => { if (document.visibilityState !== "visible" || recording || pending) return; const response = await fetch("/api/whatsapp/pulse", { cache: "no-store" }).catch(() => null); const payload = response?.ok ? await response.json().catch(() => null) as { version?: string } | null : null; if (payload?.version && version && payload.version !== version) router.refresh(); if (payload?.version) version = payload.version; }; void refresh(); const timer = window.setInterval(() => { void refresh(); }, 2_000); return () => window.clearInterval(timer); }, [pending, recording, router]);

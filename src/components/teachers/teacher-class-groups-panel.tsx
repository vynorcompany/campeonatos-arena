...
export function TeacherClassGroupsPanel({ teacherId, plans, groups }: { teacherId: string; plans: { id: string; name: string }[]; groups: Group[] }) {
  ...
  return <div className="teacher-groups-panel teacher-class-directory">
    {createOpen ? <div className="teacher-class-create-modal" role="presentation" onMouseDown={() => setCreateOpen(false)}><section className="section-card teacher-detail-section teacher-class-create-panel" role="dialog" aria-modal="true" aria-labelledby="teacher-class-create-title" onMouseDown={(event) => event.stopPropagation()}>
      <header><div><p className="eyebrow">NOVA TURMA</p><h2 id="teacher-class-create-title">Horários fixos e vagas</h2></div><button type="button" className="teacher-class-create-close" onClick={() => setCreateOpen(false)} aria-label="Fechar">×</button></header>
      {!plans.length ? <p className="muted">Crie ao menos um plano para este professor antes de montar uma turma.</p> : <SafeActionForm action={createClassGroupAction} className="teacher-group-create-form" resetOnSuccess successMessage="Turma criada." onSuccess={() => { setSelectedPlanIds([]); setCreateOpen(false); }} validate={() => selectedPlanIds.length ? null : "Selecione ao menos um plano para a turma."}>
        <input type="hidden" name="teacherId" value={teacherId} />
        <label>Nome da turma<input name="name" required placeholder="Ex.: Iniciante noite" /></label>
        <fieldset><legend>Plano obrigatório</legend>{plans.map((plan) => <label key={plan.id}><input type="checkbox" name="planIds" value={plan.id} checked={selectedPlanIds.includes(plan.id)} onChange={(event) => setSelectedPlanIds((current) => event.currentTarget.checked ? [...current, plan.id] : current.filter((id) => id !== plan.id))} />{plan.name}</label>)}</fieldset>

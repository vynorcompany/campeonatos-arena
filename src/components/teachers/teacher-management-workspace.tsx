...
export function TeacherManagementWorkspace({ teachers }: { teachers: Teacher[] }) {
  ...
  const visibleTeachers = teachers
    .filter((teacher) => (status === "ALL" || (status === "ACTIVE" ? teacher.active : !teacher.active)) && teacher.name.toLocaleLowerCase("pt-BR").includes(query.toLocaleLowerCase("pt-BR")))
    .sort((first, second) => order === "STUDENTS" ? second.studentAssignments.length - first.studentAssignments.length || first.name.localeCompare(second.name, "pt-BR") : order === "PLANS" ? second.planAssignments.length - first.planAssignments.length || first.name.localeCompare(second.name, "pt-BR") : first.name.localeCompare(second.name, "pt-BR"));

  return <section className="teacher-directory">
    <div className="teacher-management-toolbar"><span /><button type="button" className="button button-primary" onClick={() => setCreateOpen(true)}><span aria-hidden="true">＋</span> Novo professor</button></div>
    ...
    <div className="teacher-directory-list">{visibleTeachers.map((teacher) => { const initials = teacher.name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase(); const contact = teacher.phone || teacher.email; return <Link href={`/professores/${teacher.id}`} className="teacher-directory-item" key={teacher.id}><span className="teacher-directory-avatar" aria-hidden="true">{initials}<i className={teacher.active ? "is-active" : ""} /></span><div className="teacher-directory-person"><strong>{teacher.name}</strong>{contact ? <span className="teacher-directory-contact"><span aria-hidden="true">{teacher.phone ? "⌕" : "✉"}</span>{contact}</span> : <span>Sem contato cadastrado</span>}{!contact ? <b>Adicionar contato</b> : null}</div><span className={teacher.active ? "teacher-directory-status is-active" : "teacher-directory-status"}>{teacher.active ? "Ativo" : "Inativo"}</span><div className="teacher-directory-metrics"><span className="teacher-directory-metric"><strong>{teacher.studentAssignments.length}</strong><small>Alunos</small></span><span className="teacher-directory-metric"><strong>{teacher.planAssignments.length}</strong><small>Planos ativos</small></span></div><span className="teacher-directory-arrow" aria-label={`Abrir ${teacher.name}`}>›</span></Link>; })}{!visibleTeachers.length ? <p className="muted teacher-directory-empty">{teachers.length ? "Nenhum professor encontrado com estes filtros." : "Nenhum professor cadastrado."}</p> : null}</div>

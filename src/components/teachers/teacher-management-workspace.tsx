...
export function TeacherManagementWorkspace({ teachers, clients }: { teachers: Teacher[]; clients: Client[] }) {
  ...
  return <section className="teacher-directory">
    ...
    <div className="teacher-directory-list">{visibleTeachers.map((teacher) => { const contact = teacher.phone || teacher.email; return <Link href={`/professores/${teacher.id}`} className="teacher-directory-item" key={teacher.id}><PlayerAvatar className="teacher-directory-avatar" photoUrl={teacher.player?.photoUrl ?? ""} name={teacher.name} /><div className="teacher-directory-person"><strong>{teacher.name}</strong><span>{contact || "Sem contato cadastrado"}</span></div><span className={teacher.active ? "teacher-directory-status is-active" : "teacher-directory-status"}><i className="teacher-directory-status-dot" aria-hidden="true" />{teacher.active ? "Ativo" : "Inativo"}</span><div className="teacher-directory-metrics"><span className="teacher-directory-metric"><strong>{teacher.studentAssignments.length}</strong><small>Alunos</small></span><span className="teacher-directory-metric"><strong>{teacher.planAssignments.length}</strong><small>Planos</small></span></div><span className="teacher-directory-arrow" aria-label={`Abrir ${teacher.name}`}><EventIcon name="chevron" /></span></Link>; })}{!visibleTeachers.length ? <p className="muted teacher-directory-empty">{teachers.length ? "Nenhum professor encontrado com estes filtros." : "Nenhum professor cadastrado."}</p> : null}</div>

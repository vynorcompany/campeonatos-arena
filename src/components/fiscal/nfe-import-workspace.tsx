...
export function NfeImportWorkspace() {
  ...
  return <>
    <div className="nfe-import-start"><label className="field">Arquivo XML da NF-e<input ref={fileRef} type="file" accept=".xml,text/xml,application/xml" disabled={pending} /><small>O estoque só será alterado depois da conciliação dos produtos.</small></label><button type="button" className="button button-primary" onClick={prepare} disabled={pending}>{pending ? "Lendo XML..." : "Conciliar produtos"}</button></div>

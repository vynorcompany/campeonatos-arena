...
export default async function PublicBookingPage({ params, searchParams }: PublicBookingPageProps) {
  ...
  const [occurrences, pendingReservations] = await Promise.all([
    prisma.scheduleOccurrence.findMany({ where: { arena: { slug: params.arenaSlug }, status: { not: "CANCELED" }, startsAt: { lt: nextDay }, endsAt: { gt: selectedDate } }, include: { occurrenceCourts: true }, orderBy: { startsAt: "asc" } }),
    currentClient ? prisma.scheduleOccurrence.findMany({ where: { arena: { slug: params.arenaSlug }, sourceType: "ONLINE_BOOKING", status: "PENDING_CONFIRMATION", participants: { some: { playerId: currentClient.playerId } }, endsAt: { gte: selectedDate } }, include: { occurrenceCourts: true }, orderBy: { startsAt: "asc" }, take: 5 }) : Promise.resolve([])
  ]);

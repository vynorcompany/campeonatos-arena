...
function bookingEventCopy(title: string, bookingTypeName: string) {
  const prefix = `${bookingTypeName} · `;
  if (title.startsWith(prefix)) return { primary: title.slice(prefix.length) || bookingTypeName, secondary: bookingTypeName };
  return { primary: title, secondary: bookingTypeName };
}

function bookingTypeClass(bookingTypeName: string) {
  const type = bookingTypeName.trim().toLowerCase();
  if (type === "aula fixa") return "agenda-event-type-fixed-lesson";
  if (type === "aula") return "agenda-event-type-lesson";
  if (type === "plano") return "agenda-event-type-plan";
  if (type === "super 12") return "agenda-event-type-super12";
  if (type === "liga") return "agenda-event-type-league";
...
export default async function AgendaPage({ searchParams }: AgendaPageProps) {
  ...
  return <div className="agenda-page" aria-label="Agenda de quadras">
    ...
    {courts.length ? <div className="daily-court-grid-scroll"><table className="daily-court-grid"><thead><tr><th scope="col">Hora</th>{courts.map((court) => <th scope="col" className="daily-court-heading" style={{ "--court-color": court.color } as CSSProperties} key={court.id}>{court.name}</th>)}</tr></thead><tbody>{slots.map((slot) => <tr key={slot}><th scope="row">{minuteLabel(slot)}</th>{courts.map((court) => {
      ...
      if (occurrence) { const rows = Math.max(1, Math.ceil((minuteOfDay(occurrence.endsAt) - minuteOfDay(occurrence.startsAt)) / arena.scheduleSlotMinutes)); const onlineBooking = occurrence.sourceType === "ONLINE_BOOKING"; const pendingOnlineConfirmation = onlineBooking && occurrence.status === "PENDING_CONFIRMATION"; const eventCopy = bookingEventCopy(occurrence.title, occurrence.bookingTypeName); return <td className={`${onlineBooking ? "daily-court-event daily-court-event-online" : "daily-court-event"} ${bookingTypeClass(occurrence.bookingTypeName)}`} key={court.id} rowSpan={rows}><div className="agenda-event-content"><AgendaSlotDialog players={players} courts={courtOptions} teachers={teachers} bookingTypes={bookingTypes} slot={{ occurrenceId: occurrence.id, courtId: court.id, courtIds: occurrence.occurrenceCourts.map((entry) => entry.courtId), courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(minuteOfDay(occurrence.startsAt)), endsAt: minuteLabel(minuteOfDay(occurrence.endsAt)), state: "OCCUPIED", pendingConfirmation: pendingOnlineConfirmation, pricingRules: court.weeklyRules, bookingTypeName: occurrence.bookingTypeName, notes: occurrence.notes, teacherId: occurrence.teacherId ?? undefined, responsiblePlayerId: occurrence.responsiblePlayerId ?? undefined, participants: occurrence.participants.map((participant) => ({ playerId: participant.playerId, amountCents: participant.amountCents, paymentMethod: participant.paymentMethod })) }}><div className="agenda-booking-event-card"><span className="agenda-booking-type-icon" title={occurrence.bookingTypeName}>{bookingTypeIcon(occurrence.bookingTypeName)}</span><div><strong>{eventCopy.primary}</strong><span className="agenda-booking-type-label">{eventCopy.secondary}</span>{pendingOnlineConfirmation ? <span className="agenda-pending-confirmation"><i aria-hidden="true">◷</i>Aguardando confirmação</span> : null}</div>{occurrence.participants.some((participant) => participant.paymentMethod) ? <span className="agenda-payment-indicator" title="Pagamento confirmado">$</span> : null}</div></AgendaSlotDialog></div></td>; }
      ...
      return <td className="daily-court-available" key={court.id}><AgendaSlotDialog players={players} courts={courtOptions} teachers={teachers} bookingTypes={bookingTypes} slot={{ courtId: court.id, courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(slot), endsAt: minuteLabel(slot + arena.scheduleSlotMinutes), state: "AVAILABLE", priceCents: rule.priceCents, pricingRules: court.weeklyRules }}><span>Livre</span></AgendaSlotDialog></td>;

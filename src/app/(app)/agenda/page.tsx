...
export default async function AgendaPage({ searchParams }: AgendaPageProps) {
  ...
  return <div className="agenda-page" aria-label="Agenda de quadras">
    ...
    {courts.length ? <div className="daily-court-grid-scroll"><table className="daily-court-grid"><thead><tr><th scope="col">Hora</th>{courts.map((court) => <th scope="col" className="daily-court-heading" style={{ "--court-color": court.color } as CSSProperties} key={court.id}>{court.name}</th>)}</tr></thead><tbody>{slots.map((slot) => <tr key={slot}><th scope="row">{minuteLabel(slot)}</th>{courts.map((court) => {
      ...
      if (occurrence) { const rows = Math.max(1, Math.ceil((minuteOfDay(occurrence.endsAt) - minuteOfDay(occurrence.startsAt)) / arena.scheduleSlotMinutes)); return <td className="daily-court-event" key={court.id} rowSpan={rows}><AgendaSlotDialog players={players} courts={courtOptions} teachers={teachers} bookingTypes={bookingTypes} slot={{ occurrenceId: occurrence.id, courtId: court.id, courtIds: occurrence.occurrenceCourts.map((entry) => entry.courtId), courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(minuteOfDay(occurrence.startsAt)), endsAt: minuteLabel(minuteOfDay(occurrence.endsAt)), state: "OCCUPIED", bookingTypeName: occurrence.bookingTypeName, notes: occurrence.notes, teacherId: occurrence.teacherId ?? undefined, participants: occurrence.participants.map((participant) => ({ playerId: participant.playerId, amountCents: participant.amountCents, paymentMethod: participant.paymentMethod })) }}><strong>{occurrence.title}{occurrence.participants.some((participant) => participant.paymentMethod) ? <span className="agenda-payment-indicator" title="Pagamento confirmado">$</span> : null}</strong><span>{occurrence.sourceType}</span></AgendaSlotDialog></td>; }

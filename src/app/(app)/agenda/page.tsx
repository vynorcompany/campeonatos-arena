...
export default async function AgendaPage({ searchParams }: AgendaPageProps) {
  ...
  return <div className="agenda-page" aria-label="Agenda de quadras">
    ...
    {courts.length ? <div className="daily-court-grid-scroll"><table className="daily-court-grid"><thead><tr><th scope="col">Hora</th>{courts.map((court) => <th scope="col" key={court.id}>{court.name}</th>)}</tr></thead><tbody>{slots.map((slot) => <tr key={slot}><th scope="row">{minuteLabel(slot)}</th>{courts.map((court) => {
      ...
      if (occurrence) { const rows = Math.max(1, Math.ceil((minuteOfDay(occurrence.endsAt) - minuteOfDay(occurrence.startsAt)) / arena.scheduleSlotMinutes)); return <td className="daily-court-event" key={court.id} rowSpan={rows}><AgendaSlotDialog players={players} bookingTypes={bookingTypes} slot={{ occurrenceId: occurrence.id, courtId: court.id, courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(minuteOfDay(occurrence.startsAt)), endsAt: minuteLabel(minuteOfDay(occurrence.endsAt)), state: "OCCUPIED", title: occurrence.title, sourceType: occurrence.sourceType, bookingTypeName: occurrence.bookingTypeName, notes: occurrence.notes, participants: occurrence.participants.map((participant) => ({ playerId: participant.playerId, amountCents: participant.amountCents, paymentMethod: participant.paymentMethod })) }}><strong>{occurrence.title}</strong><span>{occurrence.sourceType}</span></AgendaSlotDialog></td>; }
      const rule = court.weeklyRules.find((item) => item.weekday === weekday && item.startsAtMinute <= slot && item.endsAtMinute > slot);
      if (!rule || !rule.available) return <td className="daily-court-unavailable" key={court.id}><AgendaSlotDialog players={players} bookingTypes={bookingTypes} slot={{ courtId: court.id, courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(slot), endsAt: minuteLabel(slot + arena.scheduleSlotMinutes), state: "UNAVAILABLE" }}><span>Indisponível</span></AgendaSlotDialog></td>;
      return <td className="daily-court-available" key={court.id}><AgendaSlotDialog players={players} bookingTypes={bookingTypes} slot={{ courtId: court.id, courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(slot), endsAt: minuteLabel(slot + arena.scheduleSlotMinutes), state: "AVAILABLE", priceLabel: priceLabel(rule.priceCents), priceCents: rule.priceCents }}><strong>{priceLabel(rule.priceCents)}</strong><span>Livre</span></AgendaSlotDialog></td>;

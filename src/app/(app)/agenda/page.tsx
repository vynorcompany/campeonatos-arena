...
export default async function AgendaPage({ searchParams }: AgendaPageProps) {
  ...
  return <div className="agenda-page" aria-label="Agenda de quadras">
    ...
    {courts.length ? <div className="daily-court-grid-scroll"><table className="daily-court-grid"><thead><tr><th scope="col">Hora</th>{courts.map((court) => <th scope="col" key={court.id}>{court.name}</th>)}</tr></thead><tbody>{slots.map((slot) => <tr key={slot}><th scope="row">{minuteLabel(slot)}</th>{courts.map((court) => {
      ...
      return <td className="daily-court-available" key={court.id}><AgendaSlotDialog players={players} slot={{ courtId: court.id, courtName: court.name, dateLabel: selectedDateLabel, dateValue: dateInputValue(selectedDate), startsAt: minuteLabel(slot), endsAt: minuteLabel(slot + arena.scheduleSlotMinutes), state: "AVAILABLE", priceLabel: priceLabel(rule.priceCents) }}><strong>{priceLabel(rule.priceCents)}</strong><span>Livre</span></AgendaSlotDialog></td>;

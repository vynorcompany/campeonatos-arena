...
export default async function TvPresentationSettingsPage() {
  ...
  return <div className="stack-md">
    <header className="page-header"><h1>Tela Da TV</h1><Link href="/proximos-jogos/tv" className="button button-primary" target="_blank" rel="noreferrer">Abrir TV</Link></header>
    <SectionCard title="Slides exibidos" description="As logos selecionadas aparecem no carrossel. Premiações são definidas nas ligas e eventos ativos.">
      <SafeActionForm action={upsertTvPresentationSettingsAction} className="grid-form" successMessage="Configurações da TV salvas.">
        ...
        <div className="field form-full tv-event-source-field"><label htmlFor="selected-tournament">Premiação exibida</label><select id="selected-tournament" name="selectedTournamentId" defaultValue={payload.settings.selectedTournamentId}><option value="">Não exibir premiação</option>{tournaments.map((tournament) => <option key={tournament.id} value={tournament.id}>Evento · {tournament.name} ({tournament.status})</option>)}{leagueCycles.map((cycle) => <option key={cycle.id} value={`league:${cycle.id}`}>Liga · {cycle.competition.category.name}{cycle.prizeDescription ? "" : " (sem premiação cadastrada)"}</option>)}</select></div>
        <div className="tv-settings-grid form-full"><label className="check-option tv-check-option"><input name="showSponsors" type="checkbox" {...checked(payload.settings.showSponsors)} /><span>Patrocinadores</span></label><label className="check-option tv-check-option"><input name="showRanking" type="checkbox" {...checked(payload.settings.showRanking)} /><span>Ranking do evento ativo</span></label><label className="check-option tv-check-option"><input name="showMonthlyPrize" type="checkbox" {...checked(payload.settings.showMonthlyPrize)} /><span>Premiações de eventos</span></label></div>
        <div className="form-full tv-sponsor-picker"><div className="split-row"><div><strong>Patrocinadores exibidos</strong><p className="muted">Selecione as marcas que entram na rotação da TV.</p></div><Link href="/proximos-jogos/patrocinios" className="button button-secondary">Gerenciar patrocínios</Link></div>{payload.availableSponsors.length ? <TvSponsorMultiselect sponsors={payload.availableSponsors} selectedSponsorIds={payload.settings.selectedSponsorIds} /> : <p className="muted">Cadastre patrocinadores para selecioná-los aqui.</p>}</div>

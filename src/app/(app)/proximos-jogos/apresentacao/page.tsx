  ...
  return <div className="stack-md">
    ...
    <SectionCard title="Slides exibidos" description="As logos selecionadas aparecem no carrossel. Premiações são definidas nas ligas e eventos ativos.">
      <SafeActionForm action={upsertTvPresentationSettingsAction} className="grid-form" successMessage="Configurações da TV salvas.">
        ...
        <div className="form-full tv-sponsor-picker"><div className="split-row"><div><strong>Patrocinadores exibidos</strong><p className="muted">Selecione apenas as marcas que entram na rotação da TV.</p></div><Link href="/proximos-jogos/patrocinios" className="button button-secondary">Gerenciar patrocínios</Link></div><div className="tv-settings-grid">{payload.availableSponsors.length ? payload.availableSponsors.map((sponsor) => <label key={sponsor.id} className="check-option tv-check-option"><input name="selectedSponsorIds" type="checkbox" value={sponsor.id} defaultChecked={payload.settings.selectedSponsorIds.includes(sponsor.id)} /><span>{sponsor.name}</span></label>) : <p className="muted">Cadastre patrocinadores para selecioná-los aqui.</p>}</div></div>

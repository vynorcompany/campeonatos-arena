...
export default async function OverviewPage({ searchParams }: { searchParams: DashboardSearchParams }) {
  ...
  const comparisonMode = query.comparar === "previous-year" ? "previous-year" : "previous-period";
  const previousTo = comparisonMode === "previous-year" ? new Date(to) : new Date(from);
  const previousFrom = comparisonMode === "previous-year" ? new Date(from) : new Date(previousTo);
  ...
  else { previousTo.setMilliseconds(-1); previousFrom.setDate(previousFrom.getDate() - periodDays + 1); previousFrom.setHours(0, 0, 0, 0); }
  ...
  const comparing = query.comparar === "previous-period" || query.comparar === "previous-year";

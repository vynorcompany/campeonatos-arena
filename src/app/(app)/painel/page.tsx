...
type DashboardSearchParams = Promise<{ dataInicial?: string; dataFinal?: string; visao?: string; comparar?: string }>;
...
export default async function OverviewPage({ searchParams }: { searchParams: DashboardSearchParams }) {
  ...
  const previousTo = new Date(from); previousTo.setMilliseconds(-1);
  const previousFrom = new Date(previousTo); previousFrom.setDate(previousFrom.getDate() - periodDays + 1); previousFrom.setHours(0, 0, 0, 0);
  ...
  const presetHref = (start: Date, end: Date) => `/painel?dataInicial=${inputDate(start)}&dataFinal=${inputDate(end)}&visao=${view}`;
  ...
  return <div className="stack-md workspace-page dashboard-analytics-page">
    <header className="page-header dashboard-header"><div><p className="eyebrow">VISÃO GERAL</p><h1>Saúde da arena</h1></div><form className="dashboard-filters"><div className="dashboard-presets"><Link href={presetHref(thisMonthStart, today)}>Este mês</Link><Link href={presetHref(lastMonthStart, lastMonthEnd)}>Mês passado</Link><Link href={presetHref(new Date(today.getFullYear(), today.getMonth(), today.getDate() - 89), today)}>Últimos 90 dias</Link><Link href={presetHref(new Date(today.getFullYear(), today.getMonth(), today.getDate() - 6), today)}>Últimos 7 dias</Link></div><label>Período inicial<input type="date" name="dataInicial" defaultValue={inputDate(from)} /></label><label>Período final<input type="date" name="dataFinal" defaultValue={inputDate(to)} /></label><label>Visualização<select name="visao" defaultValue={view}><option value="caixa">Visão de caixa</option><option value="competencia">Visão de competência</option></select></label><label className="dashboard-compare-toggle">Comparar<input type="checkbox" name="comparar" value="1" defaultChecked={query.comparar === "1"} /></label><button type="submit" className="button button-primary">Aplicar</button></form></header>
    <SectionCard title="Resumo financeiro" className="dashboard-financial-summary"><div className="stats-grid"><StatCard label="Entradas" value={money(received)} caption={view === "caixa" ? "Valores recebidos" : "Receitas previstas"} comparison={query.comparar === "1" ? { percent: comparisonPercent(received, previousReceived) } : undefined} /><StatCard label="Saídas" value={money(paid)} caption={view === "caixa" ? "Valores pagos" : "Despesas previstas"} comparison={query.comparar === "1" ? { percent: comparisonPercent(paid, previousPaid) } : undefined} /><StatCard label="Resultado" value={money(received - paid)} caption={view === "caixa" ? "Saldo do período" : "Resultado por competência"} comparison={query.comparar === "1" ? { percent: comparisonPercent(received - paid, previousReceived - previousPaid) } : undefined} /><StatCard label="Reservas" value={reservations.length} caption="No período" comparison={query.comparar === "1" ? { percent: comparisonPercent(reservations.length, previousReservations.length) } : undefined} /></div></SectionCard>

...
export default async function WhatsAppPage() {
  ...
  return <div className="stack-md whatsapp-page"><WhatsAppChatWorkspace connected={connection?.status === "CONNECTED"} slaMinutes={arena.whatsappSlaMinutes} clients={clients} conversations={conversations.map((conversation) => ({ ...conversation, lastMessageAt: conversation.lastMessageAt.toISOString(), archivedAt: conversation.archivedAt?.toISOString() ?? null, messages: conversation.messages.map((message) => ({ ...message, sentAt: message.sentAt.toISOString() })) }))} /></div>;
}

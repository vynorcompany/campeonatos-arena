...
export default async function ComandasPage({ searchParams }: ComandasPageProps) {
  ...
  return <div className="commands-page">
    ...
    <section className="commands-list commands-day-panel"><div className="commands-list-head"><div><span className="commands-day-heading-icon" aria-hidden="true"><CommandIcon name="calendar" /></span><strong>{formatDate(selectedDate)}</strong></div><span><i aria-hidden="true"><CommandIcon name="list" /></i> {comandas.length} comanda{comandas.length === 1 ? "" : "s"}</span></div>{comandas.length ? <div className="commands-list-items">{comandas.map((comanda) => <CommandCard key={comanda.id} canDelete={canDeleteComandas} products={products} paymentMethods={paymentMethods} debts={comanda.playerId ? debtsByPlayer[comanda.playerId] ?? [] : []} comanda={{ id: comanda.id, code: comanda.code, label: comanda.label, type: comanda.type, playerName: comanda.player?.name, items: comanda.items }} />)}</div> : <div className="commands-empty"><div className="commands-empty-illustration" aria-hidden="true"><CommandIcon name="receipt" /></div><strong>Nenhuma comanda neste dia.</strong><span>Crie uma nova comanda para iniciar o atendimento.</span><Link href={`/comandas?${new URLSearchParams({ ...Object.fromEntries(baseParams), new: "client" })}`} className="button button-primary commands-empty-action"><CommandIcon name="plus" /> Nova Comanda</Link></div>}</section>
  </div>;
}

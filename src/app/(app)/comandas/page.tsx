...
export default async function ComandasPage({ searchParams }: ComandasPageProps) {
  ...
  const [comandas, openComandas, players, products, paymentMethodSettings, pendingDebts] = await withArenaTransaction(auth.arenaId, (tx) => Promise.all([
    tx.comanda.findMany({
      ...
      include: { player: { select: { name: true, balanceMovements: { select: { amountCents: true } } } }, items: { include: { product: { select: { name: true } } }, orderBy: { createdAt: "asc" } } }
    }),
    tx.comanda.findMany({
  ...
  return <div className="commands-page">
    ...
    <section className="commands-list commands-day-panel"><div className="commands-list-head"><div><span className="commands-day-heading-icon" aria-hidden="true"><CommandIcon name="calendar" /></span><strong>{formatDate(selectedDate)}</strong></div><span><i aria-hidden="true"><CommandIcon name="list" /></i> {comandas.length} comanda{comandas.length === 1 ? "" : "s"}</span></div>{comandas.length ? <div className="commands-list-items">{comandas.map((comanda) => <CommandCard key={comanda.id} canDelete={canDeleteComandas} products={products} paymentMethods={paymentMethods} debts={comanda.playerId ? debtsByPlayer[comanda.playerId] ?? [] : []} clientCreditCents={Math.max(0, comanda.player?.balanceMovements.reduce((total, movement) => total + movement.amountCents, 0) ?? 0)} comanda={{ id: comanda.id, code: comanda.code, label: comanda.label, type: comanda.type, playerName: comanda.player?.name, items: comanda.items }} />)}</div> : <div className="commands-empty"><div className="commands-empty-illustration" aria-hidden="true"><CommandIcon name="receipt" /></div><strong>Nenhuma comanda neste dia.</strong><span>Crie uma nova comanda para iniciar o atendimento.</span><Link href={`/comandas?${new URLSearchParams({ ...Object.fromEntries(baseParams), new: "client" })}`} className="button button-primary commands-empty-action"><CommandIcon name="plus" /> Nova Comanda</Link></div>}</section>
  </div>;
}

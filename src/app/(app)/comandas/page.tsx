...
export default async function ComandasPage({ searchParams }: ComandasPageProps) {
  ...
  return <div className="commands-page">
    ...
    <section className="commands-list"><div className="commands-list-head"><strong>{formatDate(selectedDate)}</strong><span>{comandas.length} comanda{comandas.length === 1 ? "" : "s"}</span></div>{comandas.length ? <div className="commands-list-items">{comandas.map((comanda) => <CommandCard key={comanda.id} canDelete={canDeleteComandas} products={products} paymentMethods={paymentMethods} debts={comanda.playerId ? debtsByPlayer[comanda.playerId] ?? [] : []} comanda={{ id: comanda.id, code: comanda.code, label: comanda.label, type: comanda.type, playerName: comanda.player?.name, items: comanda.items }} />)}</div> : <div className="commands-empty"><strong>Nenhuma comanda neste dia.</strong><span>Crie uma nova comanda para iniciar o atendimento.</span></div>}</section>
  </div>;
}

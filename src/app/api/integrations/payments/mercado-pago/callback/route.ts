...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (arenaId === "agency-platform") {
      await prisma.agencyPaymentConnection.upsert({
        where: { id: "platform" },
        create: { id: "platform", status: "CONNECTED", displayName: account.displayName || "Mercado Pago", accountReference: account.reference || String(token.user_id ?? ""), encryptedSecrets: encryptConnectionSecrets({ accessToken: token.access_token, refreshToken: token.refresh_token ?? "" }), expiresAt: token.expires_in ? new Date(Date.now() + token.expires_in * 1000) : null, connectedAt: new Date() },
      ...
      await issueDueAgencyInvoices().catch((error) => console.error("Agency billing preparation failed after Mercado Pago connection", error));
      return NextResponse.redirect(new URL("/agencia/planos?connected=mercado-pago", applicationUrl));
    }
    ...
    const agency = url.searchParams.get("state") ? (() => { try { return readMercadoPagoOAuthState(url.searchParams.get("state")!) === "agency-platform"; } catch { return false; } })() : false;
    const path = agency ? "/agencia/planos" : "/financeiro/configuracoes/pagamentos-online";
    return NextResponse.redirect(new URL(`${path}?connectionError=${encodeURIComponent(error instanceof Error ? error.message : "Não foi possível concluir a conexão.")}`, applicationUrl));
  }
}

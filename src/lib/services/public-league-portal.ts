...
export async function getPublicLeaguePortal(arenaSlug: string, playerId: string, requestedCategoryId?: string) {
  ...
  const challenges = await prisma.leagueMatchProposal.findMany({ where: { OR: [{ proposerPairId: { in: ownPairs.map((pair) => pair.id) } }, { opponentPairId: { in: ownPairs.map((pair) => pair.id) } }] }, include: { court: { select: { name: true } }, categoryMatch: { include: { competition: { select: { categoryId: true } }, homePair: { select: { name: true } }, awayPair: { select: { name: true } } } } }, orderBy: { createdAt: "desc" } });
  const now = new Date();
          ...
          include: {
            student: {
              select: {
                ...
                monthlyBalances: { where: { referenceMonth: `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}` }, take: 1 },
                attendances: { where: { makeupRequestedAt: { not: null }, makeupScheduledAt: null, makeupExpiresAt: { gte: now } }, select: { id: true, status: true } },
              },
            },
          },
  ...
  const leagueCompetitions = await prisma.categoryCompetition.findMany({
    where: { format: "LEAGUE", status: { not: "FINISHED" }, category: { active: true, tournament: { arenaId: arena.id } }, OR: [{ status: "PUBLISHED" }, { pairs: { some: { active: true, players: { some: { playerId } } } } }] },
  ...
  const latestOwnCompetition = [...ownPairs].sort((left, right) => right.createdAt.getTime() - left.createdAt.getTime()).map((pair) => leagueCompetitions.find((competition) => competition.id === pair.competitionId)).find(Boolean);
  const selectedLeagueCompetition = leagueCompetitions.find((competition) => competition.categoryId === requestedCategoryId) ?? latestOwnCompetition ?? leagueCompetitions[0] ?? null;
  ...
  return {
    ...
    challenges: challenges.map((challenge) => ({ id: challenge.id, categoryId: challenge.categoryMatch.competition.categoryId, status: challenge.status, opponent: challenge.categoryMatch.awayPair?.name ?? "Dupla visitante", proposer: challenge.categoryMatch.homePair?.name ?? "Dupla mandante", court: challenge.court.name, proposedAt: dateTimeLabel(challenge.startsAt), responseDueAt: dateTimeLabel(challenge.responseDueAt), block: challenge.categoryMatch.leagueBlock, incoming: ownPairs.some((pair) => pair.id === challenge.opponentPairId) })),
    leagueNotifications: leagueNotifications.map((notification) => {
      const proposalId = notification.href.match(/#desafio-([^#?]+)/)?.[1];
      const challenge = proposalId ? challenges.find((item) => item.id === proposalId) : null;
      const href = challenge ? `/classificacao/${arenaSlug}?section=leagues&tab=games&leagueTab=games&leagueCategory=${challenge.categoryMatch.competition.categoryId}#desafio-${challenge.id}` : notification.href;
    ...
    teacherManagement: teacherManagement ? {
      ...
      students: teacherManagement.studentAssignments.map((assignment) => { const initialMonthlyClasses = assignment.student.subscriptions[0]?.classesPerMonth ?? assignment.student.monthlyBalances[0]?.totalClasses ?? assignment.student.remainingClasses; return { id: assignment.student.id, name: assignment.student.name, remainingClasses: Math.min(assignment.student.monthlyBalances[0]?.remainingClasses ?? initialMonthlyClasses, initialMonthlyClasses), initialMonthlyClasses, planName: assignment.student.subscriptions[0]?.plan.name ?? "Sem plano ativo", classGroup: assignment.student.classGroupEnrollments[0]?.classGroup ?? null, requestedMakeups: assignment.student.attendances.length, pendingMakeups: assignment.student.attendances.filter((attendance) => attendance.status === "ABSENT").map((attendance) => attendance.id) }; }),
      makeupSlots,

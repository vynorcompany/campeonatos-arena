...
export async function getPublicClientHome(arenaSlug: string, playerId: string) {
  ...
  const now = new Date();
  const [announcements, events, eventPosts, student, reservations, pairs, entries, player] = await withArenaTransaction(arena.id, (tx) => Promise.all([
    ...
    tx.portalEventPost.findMany({ where: { arenaId: arena.id, active: true }, orderBy: { createdAt: "desc" }, take: 6, select: { id: true, title: true, caption: true, imageUrl: true } }),
    ...
    tx.financialEntry.findMany({ where: { arenaId: arena.id, type: "REVENUE", status: { in: ["PENDING", "OVERDUE"] }, counterpartyName: { not: "" } }, select: { counterpartyName: true, amountCents: true } }),
    tx.player.findUnique({ where: { id: playerId }, select: { name: true } })
  ]));

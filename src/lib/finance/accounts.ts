...
export async function getAccountsLedger(arenaId: string, type: "REVENUE" | "EXPENSE", filters: LedgerFilters = {}) {
  ...
  const entries = await prisma.financialEntry.findMany({ where, orderBy: [{ [dateField]: "asc" }, { createdAt: "asc" }], include: { settlements: { select: { amountCents: true, interestCents: true } }, scheduleParticipant: { select: { player: { select: { name: true } } } }, sale: { select: { customerName: true, comanda: { select: { label: true } } } }, supplier: { select: { name: true } } } });
  return entries.map((entry) => ({ id: entry.id, counterpartyName: entry.counterpartyName || entry.supplier?.name || entry.scheduleParticipant?.player.name || entry.sale?.comanda?.label || entry.sale?.customerName || "Não informado", category: entry.category, description: entry.description, amountCents: entry.amountCents, dueDate: entry.dueDate ? entry.dueDate.toISOString().slice(0, 10) : null, status: entry.status, voidReason: entry.voidReason, balance: getFinancialEntryBalance(entry.amountCents, entry.settlements, entry.status) }));
}
